...
llvm::GenericValue CodeGenContext::runCode() {
	...
	/*-- Here we created one function for type checking its basically coonected with llvm package. We take type information as an argument as string which come from Grammar (type (type = here void/int/double) as a string passed from type (type = here non-terminal in Production rule in parser.y)) and then calling appropriate type function (defined in codegen.hpp). Then it sets appropriate context and return.	
	 -- For this changes we take reference from 
     -- "https://llvm.org/doxygen/classllvm_1_1Type.html"	*/

static llvm::Type *typeOf(CodeGenContext &context,const std::string &name) {
...
llvm::Value* NInteger::codeGen(CodeGenContext& context) const {
	...
	/*llvm functional block for double type defined with reference to above int type block.*/
llvm::Value* NDouble::codeGen(CodeGenContext& context) const {
    std::cerr << "Creating double: " << value << std::endl;
    return llvm::ConstantFP::get(context.getDoubleType(),
            value);
}
...
llvm::Value* NReturnStatement::codeGen(CodeGenContext& context) const {
    ...
    //abhi
    /*apply if else condition for return statement for void and integer type*/
    ...
    if(expression != NULL) {
        ...
        llvm::ReturnInst::Create(context.MyContext, NULL, context.currentBlock());
    }
    // after a return statement, no other statements should be produced;
    // return nullptr to make caller aware of that fact
    return nullptr;

    /* for return statement it is possible to return null value or integer value when we introduce void type
        so for that he we need to add if else condition and every type its check weatger type is void or integer 
        if type is void than its simply return null value other wise its return "returnValue" which get from expression*/
    /******************************/
}
...
llvm::Value* NExternDeclaration::codeGen(CodeGenContext& context) const {
	...
	/*-- Vatsal --*/
	/*-- For NExternDeclaration and NFunctionDeclaration Function type made generic(inclusion of void and double). */
