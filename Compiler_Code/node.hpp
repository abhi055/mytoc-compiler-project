...
#define PNODE(type) type*
	...
	/*-- Added variadic macro in order to create new node with variable multiple numbers of arguments. 
	  -- Similar to defined by Prof. Faber on Commit 8192db2b
...
public:
    ...
    /*-- Vatsal --*/
	/*-- Class for double type defined similarly with reference to above integer class definition.*/
...
public:
    ...
    //ABHI
    /*null value retun for void type */
    NReturnStatement() {
      expression = NULL;
    }


    /*whenever type of function is void at that time this retun statement return nothing
      its take the valur from expression but check type of function and from that it decide weather it will return value or NULL*/
...
public:
    ...
    NExternDeclaration(PNODE(const NIdentifier) type, PNODE(const NIdentifier) id,
   ...
   virtual llvm::Value* codeGen(CodeGenContext& context) const;
	...
	/*-- Did together, Abhi and Vatsal --*/
	/*-- "type" attribute added in Function declaration class in order to introduce different data type,
	  -- Now we have data type attribute (information of type) inside Functiondeclaration */
...
public:
    ...
    NFunctionDeclaration(PNODE(const NIdentifier) type, PNODE(const NIdentifier) id,
            const NVariableList &arguments,
            PNODE(const NBlock) block) :
            type(type),
